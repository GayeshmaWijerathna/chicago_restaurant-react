...
export class AdminPage extends Component<{}, State> {
    ...
    handleUpdateItem = async (id: number, updatedData: Item) => {
        try {
            ...
            const res = await axios.put(`http://localhost:4000/${id}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
            ...
            <table className="min-w-full bg-white">
                ...
                {items.map((item) => (
                    <tr key={item.id}>
                        ...
                        {/*<td className="py-2"><img src={item.image} alt={""} className="w-16 h-16 object-cover" /></td>*/}
                        <td className="py-2">
                            <button className="text-blue-500 mr-2" >  {/*btn edit*/}
